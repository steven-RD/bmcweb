...
namespace crow {
    namespace openbmc_mapper {
                    ...
                    if (thisInterfaceName != nullptr) {
                        ...
                        while (methodNode != nullptr) {
                            ...
                            if (thisMethodName != nullptr &&
                                ...
                                nlohmann::json::const_iterator argIt = transaction->arguments.begin();
                                ...
                                crow::connections::systemBus->async_send(
                                        ...
                                        sdbusplus::message::message & m) {
                                            ...
                                            handleMethodResponse(transaction, m, returnType);
                                        });
                                break;
                            }
        ...
        void handlePut(const crow::Request &req, crow::Response &res,
            ...
            nlohmann::json requestDbusData = nlohmann::json::parse(req.body, nullptr, false);
            ...
            crow::connections::systemBus->async_method_call(
                    ...
                    const GetObjectType & object_names) {
                        ...
                        for (const std::pair<std::string, std::vector < std::string >> connection : object_names) {
                                ...
                                while (ifaceNode != nullptr) {
                                    ...
                                    while (propNode != nullptr) {
                                        ...
                                        if (propertyName == transaction->propertyName) {
                                            ...
                                            if (argType != nullptr) {
                                                sdbusplus::message::message m = crow::connections::systemBus ->new_method_call(
                                                    connectionName.c_str(), transaction->objectPath.c_str(),
                                                    "org.freedesktop.DBus.""Properties","Set");
                                                    m.append(interfaceName,transaction->propertyName);
                                                    int r = sd_bus_message_open_container( m.get(), SD_BUS_TYPE_VARIANT,argType);
                                                if (r < 0) {
                                                    transaction->setErrorStatus( "Unexpected Error");
                                                    return;
                                                }
                                                r = convertJsonToDbus( m.get(), argType, transaction->propertyValue);
                                                if (r < 0) {
                                                ...
                                                crow::connections::systemBus->async_send(m,
                                                        [transaction](
                                                        boost::system::error_code ec,
                                                        sdbusplus::message::message & m) {
                                                            ...
                                                            if (ec) {
                                                                setErrorResponse( transaction->res,
                                                                        ...
                                                                        forbiddenPropDesc,"usi_PCIe switch");
                                                                        //forbiddenPropDesc,ec.message());
                                                            } else {
                                                                transaction->res.jsonValue = {
                                                                    {"status", "ok"},
                                                                    {"message", "200 OK"},
                            ...
                            connectionName, transaction->objectPath, "org.freedesktop.DBus.Introspectable", "Introspect");
                        }
                    },
            "xyz.openbmc_project.ObjectMapper",
            "/xyz/openbmc_project/object_mapper",
        ...
        inline void handleDBusUrl(const crow::Request &req, crow::Response &res,
            ...
            if (attrPosition != objectPath.npos) {
                destProperty = objectPath.substr(attrPosition + strlen(attrSeperator),
                        objectPath.length()); ///截取"/attr/"后的属性或方法
                objectPath = objectPath.substr(0, attrPosition); ///截取"/attr/"之前的路径
            }
            ...
            if (req.method() == "POST"_method) {
                ...
                if (actionPosition != objectPath.npos) {
                    std::string postProperty =
                            ...
                            objectPath.length()); ///截取"/action/"后的属性或方法
                    objectPath = objectPath.substr(0, actionPosition); ///截取"/action/"之前的路径
